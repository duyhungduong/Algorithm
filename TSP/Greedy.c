...
void read_file(char file_name[], canh a[], int *n){
    ...
    for ( i = 0; i < *n; i++)
    {
        for(j = i ; j < *n; j++ ){
            if(i==j)
                for(t=0; t<=j ;t++) fscanf(f, "%lf" , &giatriboqua );
            else{
...
int main(){
    canh dscanh[size];
    int n ;
    read_file("TSPdata.txt" , dscanh , &n);
    printf("Danh sach canh cua do thi theo thu tu la\n");
    

    bubblesort(dscanh, n*(n-1)/2);
//Mang pa[] de luu cac phuong an
    canh pa[size];
    greedy(dscanh, n*(n-1)/2 , pa);
    printf("Phuong an\n");
    in_ds_canh(pa,n , 1 );
